...
@section('page_content_body')
    ...
    <div class="panel panel-white">
        <div class="panel-heading">
            <h6 class="panel-title text-teal text-semibold">{!! __('CÁC THƯƠNG HIỆU') !!}</h6>
        </div>
                ...
                <form method="post" data-edit-mode="">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="control-label text-semibold">{!! __('Logo thương hiệu') !!}</label>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $modal_add.find('form').submit(function(){
                var form = this;
                var data = {
                    'logo_id' : $(form).find('input[name=logo_id]').val(),
                };
                var edit_mode = $(form).data('edit-mode');
                var url = '{!! route('backend.salon.brand.store', ['salon'=>$salon]) !!}';
                if (edit_mode) {
                    url = '{!! route('backend.salon.brand.update', ['salon'=>$salon]) !!}';
                    data._method = 'put';
            ...
            function addEditBrand($brand){
                ...
                var logo_id = '';
                if ($brand){
                    $modal_add.find('.modal-title').html('{!! __('SỬA LOGO THƯƠNG HIỆU') !!}');
                    $modal_add.find('form').data('edit-mode', true);
                    logo_id = $brand.logo_id;
                    $modal_add.find('form').data('edit-target', $brand.id);
                }
                else{
                    $modal_add.find('.modal-title').html('{!! __('THÊM LOGO THƯƠNG HIỆU') !!}');
                    $modal_add.find('form').data('edit-mode', false);
                }
                {!! $avatar_field->getHtmlTemplateHandleID() !!}(picker, logo_id);
            ...
            var $table_column_handles = [
                ...
                },
                {
                    data: 'avatar',
                    name: 'avatar',
                    title_html: '<th>{!! __('Logo thương hiệu') !!}</th>',
                    cell_define: {
                        orderable: false,
                        searchable: false,
                        render: function (data, type, row) {
                            var html ='<img width="120" src="'+data+'" class="edit-brand cursor-pointer" />';
                            return html;
                        }
                    }
                },
            ...
            var $options = {
                select: false,
                autoWidth: false,
                ajax: '{!! route('backend.salon.brand.edit', ['salon' => $salon]) !!}',
                columns: [],
                columnDefs: []
            };
            ...
            function deleteBrand(ids){
                if (ids.length == 0) {
                    swal({
                        ...
                        text: "{!! __('Vui lòng chọn 1 logo thương hiệu để xóa') !!}",
                ...
                swal({
                        title: "{!! __('Xóa brand') !!}",
                        text: "{!! __('Bạn có chắc chắn muốn xóa logo thương hiệu này không?') !!}",
                        type: "warning",
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                url: '{!! route('backend.salon.brand.destroy', ['salon' => $salon]) !!}',
                                dataType: 'json',
                                type: 'post',
                                data: {
                                    ids: ids,
                                ...
                                success: function () {
                                    new PNotify({
                                        ...
                                        text: "{!! __('Xóa logo thương hiệu thành công') !!}",
                                        addclass: 'bg-success stack-bottom-right',
                                        stack: {"dir1": "up", "dir2": "left", "firstpos1": 25, "firstpos2": 25},
                                        buttons: {
                                            sticker: false
