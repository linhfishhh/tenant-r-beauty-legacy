...
@section('page_content_body')
    ...
    <div class="panel panel-white">
        <div class="panel-heading">
            <h6 class="panel-title text-teal text-semibold">{!! __('CÁC ẢNH GIỚI THIỆU') !!}</h6>
        </div>
                ...
                <form method="post" data-edit-mode="">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="control-label text-semibold">{!! __('Ảnh giới thiệu') !!}</label>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $modal_add.find('form').submit(function(){
                ...
                var url = '{!! route('backend.salon.gallery.store', ['salon'=>$salon]) !!}';
                if (edit_mode) {
                    url = '{!! route('backend.salon.gallery.update', ['salon'=>$salon]) !!}';
                    data._method = 'put';
            ...
            var $options = {
                select: false,
                autoWidth: false,
                ajax: '{!! route('backend.salon.gallery.edit', ['salon' => $salon]) !!}',
                columns: [],
                columnDefs: []
            };
            ...
            function deleteBrand(ids){
                if (ids.length == 0) {
                    swal({
                        ...
                        text: "{!! __('Vui lòng chọn 1 ảnh để xóa') !!}",
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                url: '{!! route('backend.salon.gallery.destroy', ['salon' => $salon]) !!}',
                                dataType: 'json',
                                type: 'post',
                                data: {
                                    ids: ids,
