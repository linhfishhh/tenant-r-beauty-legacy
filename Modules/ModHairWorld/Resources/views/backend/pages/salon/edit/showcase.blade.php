...
@section('page_content_body')
    ...
    <div class="panel panel-white">
        <div class="panel-heading">
            <h6 class="panel-title text-teal text-semibold">{!! __('CÁC TÁC PHẨM NỔI BẬT') !!}</h6>
        </div>
                ...
                <form method="post" data-edit-mode="">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="control-label text-semibold">{!! __('Tên tác phẩm') !!}</label>
                            <input class="form-control" spellcheck="false" name="name" value="" placeholder="{!! __('Nhập tên tác phẩm') !!}">
                        </div>
                        <div class="form-group">
                            <label class="control-label text-semibold">{!! __('Danh sách ảnh') !!}</label>
                            <div class="avatar-picker" style="max-height: 300px; overflow-y: scroll; overflow-x: hidden">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $modal_add.find('form').submit(function(){
                ...
                var url = '{!! route('backend.salon.showcase.store', ['salon'=>$salon]) !!}';
                if (edit_mode) {
                    url = '{!! route('backend.salon.showcase.update', ['salon'=>$salon]) !!}';
                    data._method = 'put';
            ...
            var $table_column_handles = [
                ...
                },
                {
                    ...
                    cell_define: {
                        ...
                        render: function (data, type, row) {
                            var html ='<img src="'+data+'" class="full-width edit-showcase cursor-pointer" />';
                            return html;
                        }
                    }
                },
                {
                    data: 'name',
                    name: 'name',
                    title_html: '<th>{!! __('Tên tác phẩm') !!}</th>',
                    cell_define: {
                        render: function (data, type, row) {
                            return '<div><span class="edit-showcase cursor-pointer text-semibold text-teal">'+data+'</div>';
                        }
                    }
                },
                {
                ...
                },
                {
                    ...
                    cell_define: {
                        ...
                        width: "150px",
                        render: function (data, type, row) {
                            var html = '<a data-id="'+row.id+'" class="label label-warning label-icon cursor-pointer delete-showcase mr-10"><i class="icon-trash"></i></a>';
                            html += '<a data-id="'+row.id+'" class="edit-showcase label label-success label-icon cursor-pointer edit-stylist"><i class="icon-pencil5"></i></a>';
                            return html;
                        }
                    }
                },
            ...
            var $options = {
                select: false,
                autoWidth: false,
                ajax: '{!! route('backend.salon.showcase.edit', ['salon' => $salon]) !!}',
                columns: [],
                columnDefs: []
            };
            ...
            $table.on('draw', function (e, settings) {
                ...
                $(e.target).find('.delete-showcase').click(function () {
                    var ids = [];
                    var id = $(this).data('id');
                    ids.push(id);
                    deleteStylist(ids);
                });
                $(e.target).find('.edit-showcase').click(function () {
            ...
            function deleteStylist(ids){
                if (ids.length == 0) {
                    swal({
                        ...
                        text: "{!! __('Vui lòng chọn 1 tác phẩm để xóa') !!}",
                ...
                swal({
                        title: "{!! __('Xóa tác phẩm') !!}",
                        text: "{!! __('Bạn có chắc chắn muốn xóa tác phẩm này không?') !!}",
                        type: "warning",
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                url: '{!! route('backend.salon.showcase.destroy', ['salon' => $salon]) !!}',
                                dataType: 'json',
                                type: 'post',
                                data: {
                                    ids: ids,
                                ...
                                success: function () {
                                    new PNotify({
                                        ...
                                        text: "{!! __('Xóa tác phẩm thành công') !!}",
                                        addclass: 'bg-success stack-bottom-right',
                                        stack: {"dir1": "up", "dir2": "left", "firstpos1": 25, "firstpos2": 25},
                                        buttons: {
                                            sticker: false
