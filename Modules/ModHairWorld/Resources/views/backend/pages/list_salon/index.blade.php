...
@section('page_title', 'Danh sách salon tuỳ chọn')
@section('page_header_title')
<strong>Danh sách salon tuỳ chọn </strong>
@endsection
...
{{--
    ...
    <div class="panel panel-white">
        <div class="panel-heading">
            <h6 class="panel-title text-teal text-semibold">{!! __('DANH SÁCH SALON TUỲ CHỌN') !!}</h6>
            ...
            <div class="modal-content">  
                <form method="post">
                    ...
                    <div class="modal-header bg-primary">
                        ...
                        <h6 class="modal-title">{!! __('THÊM SALON TUỲ CHỌN') !!}</h6>
                    </div>
                    <div class="modal-body">
                     <!--  <select id="salon-selector" name="salon_id"></select> -->
                     <div class="form-group" >
                      <label for="usr">Name:</label>
                      <input type="text" class="form-control"  name="name" id="txtCustomName">
                  </div>
...
<script type="text/javascript">
    $(function () {
        ...
        //     placeholder: '{{__('Nhap ten')}}',
                ...
                // },
                // ajax: {
                //     url: '{!! route('backend.list_salons.salons') !!}',
                //     dataType: 'json',
        ...
        $modal_add.find('form').submit(function(e){
            e.preventDefault();
            var form = this;
            var url = '{!! route('backend.list_salons.add') !!}';
            $.ajax({
                url: url,
                type: 'post',
                dataType: 'json',
                data: {name: $('#txtCustomName').val()},
                beforeSend: function(){
        ...
        },
        {
            ...
            cell_define: {
                ...
                searchable: false,
                render: function (data, type, row) {
                    return '<div class="text-bold text-teal">'+row.created_at+'</div>'
                }
            }
        },
        {
            data: 'updated_at',
            name: 'updated_at',
            title_html: '<th>{!! __('Ngày cập nhật') !!}</th>',
            cell_define: {
                orderable: false,
                searchable: false,
                render: function (data, type, row) {
                    return '<div class="text-bold text-teal">'+row.updated_at+'</div>'
                }
            }
        },
        ...
        var $options = {
            ...
            autoWidth: false,
            search: false,
            ajax: '{!! route('backend.top_salons.list') !!}',
            columns: [],
            columnDefs: []
        };
