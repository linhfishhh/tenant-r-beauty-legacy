...
@section('page_title',__('Đăng ký chủ salon'))
@section('page_header_title')
    <strong>{{__('Khách đăng ký chủ salon')}}</strong>
@endsection
        ...
        $(function () {
            ...
            var $table_column_handles = [
                ...
                },
                {
                    data: '',
                    name: 'manager',
                    title_html: '<th>{!! __('Thông tin chủ') !!}</th>',
                    cell_define: {
                        render: function (data, type, row) {
                            var rs = '';
                            rs += '<div class="text-semibold text-teal mb-5">'+row.manager_name+'</div>';
                            rs += '<div class="mb-5"><i class="icon-phone position-left text-grey-300"></i><span>'+row.manager_phone+'</span></div>';
                            rs += '<div class="mb-5"><i class="icon-envelop position-left text-grey-300"></i><span>'+row.manager_email+'</span></div>';
                            return rs;
                        }
                    }
                },
                {
                    data: '',
                    name: 'info',
                    title_html: '<th>{!! __('Thông salon') !!}</th>',
                    cell_define: {
                        render: function (data, type, row) {
                            var rs = '';
                            rs += '<div class="text-semibold text-danger mb-5">'+row.salon_name+'</div>';
                            rs += '<div class="mb-5"><i class="icon-location3 position-left text-grey-300"></i><span>'+row.salon_address+'</span></div>';
                            rs += '<div class="text-semibold text-teal mb-5"></div>';
                            return rs;
                        }
                    }
                },
            ...
            var $options = {
                ...
                autoWidth: false,
                ajax: {
                    url: '{!! route('backend.salon_register.index') !!}',
                },
                columns: [],
                columnDefs: [],
            };
            ...
            $table.on( 'draw', function (e, settings) {
                ...
                $('#datatable_{{jSID()}} .editable-handled').editable({
                    ...
                    url: '{!! route('backend.salon_register.handle') !!}',
            ...
            function handle_rows(items, handled) {
                if (items.count() == 0){
                    swal({
                        ...
                        text: "{{__('Vui lòng chọn ít nhất 1 đăng ký để xử lý')}}",
                ...
                $.ajax({
                    ...
                    success: function () {
                        new PNotify({
                            title: '{{'XỬ LÝ THÀNH CÔNG'}}',
                            text: "{{__('Đăng ký được chọn đã xử lý thành công.')}}",
                            addclass: 'bg-success stack-bottom-right',
                            stack: {"dir1": "up", "dir2": "left", "firstpos1": 25, "firstpos2": 25},
                            buttons: {
                                sticker: false
            ...
            function delete_rows(items) {
                if (items.count() == 0){
                    swal({
                        title: "{{__('Không thể thực thi')}}",
                        text: "{{__('Vui lòng chọn ít nhất 1 đăng ký để xóa')}}",
                        confirmButtonColor: "#EF5350",
                        confirmButtonText: "{{__('Đã hiểu')}}",
                        type: "error"
                    });
                ...
                swal({
                    ...
                    function(isConfirm){
                        if (isConfirm){
                            ...
                            $.ajax({
                                url: '{!! route('backend.salon_register.destroy') !!}',
                                dataType: 'json',
                                type: 'post',
                                data: {
                                    ids: ids,
                                ...
                                success: function () {
                                    new PNotify({
                                        title: '{{'XỬ LÝ THÀNH CÔNG'}}',
                                        text: "{{__('Đăng ký được chọn đã xóa thành công.')}}",
                                        addclass: 'bg-success stack-bottom-right',
                                        stack: {"dir1": "up", "dir2": "left", "firstpos1": 25, "firstpos2": 25},
                                        buttons: {
                                            sticker: false
