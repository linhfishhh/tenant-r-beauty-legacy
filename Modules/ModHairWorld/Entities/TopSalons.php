...
class TopSalons extends Model
{
   ...
   function customsalon(){
     return $this->hasOne(CustomSalons::class, 'id', 'custom_id');
   }
   static function getTopSalons(){
       $rs = false;
       $settings = getSettingsFromPage('topsalons_configs');
       $settings = collect($settings);
       $promo_enable = $settings->get('topsalons_enable', false);

       if(!$promo_enable){
           return $rs;
       }

       $promo_date_range = $settings->get('topsalons_date_range', null);
       ...
       $promo_days = $settings->get('topsalons_days', []);

       if(!in_array($today->dayOfWeekIso.'', $promo_days)){
           return $rs;
       }

       $cats = $settings->get('topsalons_cats', []);
       if(!is_array($cats)){
           $cats = [$cats];
       }
       ...
       $promo_limit = $settings->get('topsalons_limit', 0);
       $promo_percent =  $settings->get('topsalons_percent', 0);
